    ...
    for sheet_name in xls1.sheet_names[:6]:
        if sheet_name in xls2.sheet_names:
            ...
            if set(df1.columns) != set(df2.columns):
                st.warning(f"Column names don't match in sheet {sheet_name}")
                continue
            # Iterate through the rows to compare the 'PRODUCT' column
            for index, row in df1.iterrows():
                # Find the corresponding row in the second sheet by matching 'Tank Name'
                matching_row = df2[(df2['Tank Name'] == row['Tank Name'])]
